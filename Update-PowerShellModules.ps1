...
if (!(Get-Module -Name PackageManagement)){
    ...
    Move-Item -Path "$WorkingDir\1.4.8.1" -Destination "$env:ProgramFiles\WindowsPowerShell\Modules\PackageManagement\1.4.8.1"
    }
...
if (!(Get-Module -Name PSReadline)){
    ...
    $Null = New-Item -Path "$env:ProgramFiles\WindowsPowerShell\Modules\PSReadline" -ItemType Directory -ErrorAction SilentlyContinue
    Move-Item -Path "$WorkingDir\2.3.4" -Destination "$env:ProgramFiles\WindowsPowerShell\Modules\PSReadlin\2.3.4"
    }
